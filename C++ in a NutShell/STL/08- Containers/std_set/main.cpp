...
int main(void)
{
    ...
    */
	...
	Person S1{"Fady",21} , S2{"Hady",22} , S3{"Saher",26}, S4{"Yahya",26};
	std::set<Person> Students{
		S1,S2,S3
		};

	Students.insert(S4);
	//Note that Yahya won't be inserted as it has 26 as age which is considered as a repeated value.
...
/* *************************************************** *
 * Reminder: SOMETHING IS WRONG WITH SET?????????????? *
 * Try changing the S1.name to be "Raouf" for example  *
